...
def eval_val_sliding_window(
    ...
    with torch.inference_mode():
        ...
        for batch_idx in range(num_batches):
            batch_wins = window_list[
                batch_idx * eval_batch : (batch_idx + 1) * eval_batch
            ]
            bsz = len(batch_wins)
            ...
            scored_logits = logits[:, -stride:, :].reshape(
                -1, logits.size(-1)
            )  # [B*stride, vocab]
            ...
            targets = (
                torch.stack(
                    [
                        val_tokens[
                            w * stride + seq_len - stride + 1 : w * stride + seq_len + 1
                        ]
                        for w in batch_wins
                    ]
                )
...
def quantize_float_tensor(t: Tensor) -> tuple[Tensor, Tensor]:
    ...
    if t32.ndim == 2:
        ...
        scale = (clip_abs / float(INT6_QUANT_RANGE)).clamp_min(
            1.0 / float(INT6_QUANT_RANGE)
        )
        q = (
            torch.clamp(
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training and w.ndim == 2:
            ...
            with torch.no_grad():
                ...
                w_q = (torch.round(w_clipped / scale[:, None]) * scale[:, None]).to(
                    x.dtype
                )
...
def main() -> None:
    ...
    if master_process:
        ...
        log0(
            f"Total submission size int6+{compress_name}: {quant_file_bytes + code_bytes} bytes"
        )
