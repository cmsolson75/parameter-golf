...
class Hyperparameters:
    ...
    mlp_hidden = int(
        os.environ.get("MLP_HIDDEN", 1536)
    )  # 3x model_dim, enabled by int6+zstd compression
...
def quantize_float_tensor(t: Tensor, use_int6: bool = False) -> tuple[Tensor, Tensor]:
    ...
    scale = torch.tensor(
        clip_abs / float(qmax) if clip_abs > 0 else 1.0, dtype=torch.float32
    )
    q = (
        torch.clamp(
...
def main() -> None:
    ...
    mlp_hidden = (
        args.mlp_hidden if args.mlp_hidden > 0 else args.mlp_mult * args.model_dim
    )
    ...
    if master_process:
        ...
        log0(
            f"Total submission size int6+{compress_label}: {quant_file_bytes + code_bytes} bytes"
        )
    ...
    if args.eval_stride > 0:
        ...
        log0(
            f"final_sliding_window_exact val_loss:{sw_val_loss:.8f} val_bpb:{sw_val_bpb:.8f}"
        )
